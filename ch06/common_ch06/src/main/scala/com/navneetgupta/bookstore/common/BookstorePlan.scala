  ...
  def respond(f: Future[Any], resp: unfiltered.Async.Responder[HttpResponse]) = {
    ...
    f.onComplete {
      ...
      case util.Success(EmptyResult) =>
        resp.respond(asJson(ApiResponse(ApiResponseMeta(NotFound.code, Some(ErrorMessage("notfound")), false)), NotFound))
      ...
      case util.Success(fail: Failure) =>
        ...
        val apiResp = ApiResponse(ApiResponseMeta(status.code, Some(fail.message), false))
      ...
      case util.Success(x) =>
        val apiResp = ApiResponse(ApiResponseMeta(InternalServerError.code, Some(ServiceResult.UnexpectedFailure), false))
