...
case class SalesOrderFO(override val id: Int, userId: Int, creditTxnId: Int, status: SalesOrderStatus.Value, totalCost: Double, lineItems: List[SalesOrderLineItemFO], createTs: Date, modifyTs: Date, override val deleted: Boolean = false) extends EntityFieldsObject[Int, SalesOrderFO] {
...
object SalesOrder {
  ...
  case class ResolvedDependencies(inputs: Inputs, expectedBooks: Set[Int],
                                  user: Option[UserFO], books: Map[Int, BookFO], userAssociate: ActorRef,
...
class SalesOrder(idInput: Int) extends EntityActor[SalesOrderFO](idInput) {
  ...
  override val repo = new SalesOrderRepository
